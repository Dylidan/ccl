...
class ChatController extends ChangeNotifier with WidgetsBindingObserver {
  ...
  Future<void> _initializeInternal() async {
    ...
    // FIX (Bug 3): catch errors from the background TTS init so they don't
    // become unhandled future exceptions. A failed init is already handled
    // inside PiperTtsService — it will retry on the next speak() call.
    unawaited(
      _ttsService.init().catchError((Object e) {
        if (kDebugMode) debugPrint('TTS background init failed: $e');
      }),
    );
